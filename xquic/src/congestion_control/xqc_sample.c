                    ...
                    xqc_usec_t now)
{
    ...
    sampler->interval = xqc_max(sampler->interval, 2000);
...
void xqc_update_sample(xqc_sample_t *sampler, xqc_packet_out_t *packet,
...
xqc_sample_check_app_limited(xqc_sample_t *sampler, xqc_send_ctl_t *send_ctl, xqc_send_queue_t *send_queue)
{
    ...
    if (not_cwnd_limited                                  /* We are not limited by CWND. */
        ...
        && xqc_list_empty(&send_queue->sndq_pto_probe_packets) && all_path_buffer_empty)
    {
