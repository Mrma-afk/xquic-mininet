...
void xqc_cubic_log_metrics(void *cong_ctl, xqc_packet_out_t *po, xqc_usec_t now)
{
    xqc_cubic_t *cubic = (xqc_cubic_t *)(cong_ctl);
    if (now - cubic->bw_sample_start_time >= 100000){

        if (cubic_log_fp == NULL)
        {
            cubic_log_fp = fopen("/home/mxjpxk/xquic/xquic_info/cubic_metrics.log", "a"); // 打开日志文件（只打开一次）
            if (!cubic_log_fp)
            {
                perror("Failed to open cubic_metrics.log");
                return;
            }
            fprintf(cubic_log_fp, "timestamp(us),cwnd(bytes),cubic,bandwidth(bytes/sec)\n");
        }

        double interval_sec = (now - cubic->bw_sample_start_time) / 1000000.0;
        double bandwidth_bps = (cubic->bw_sample_bytes * 8.0) / interval_sec;
        fprintf(cubic_log_fp, "%" PRIu64 ", %" PRIu64 ",%" PRIu64 ", %p, %.2f\n",
                now,
                cubic->cwnd,
                cubic->min_rtt,
                cubic,
...
{
    xqc_cubic_log_metrics(cong_ctl, po, now);
    xqc_cubic_t *cubic = (xqc_cubic_t *)(cong_ctl);
    ...
    if (cubic->bw_sample_start_time == 0) {

        cubic->bw_sample_start_time = now;
    }
    cubic->bw_sample_bytes += acked_bytes;
