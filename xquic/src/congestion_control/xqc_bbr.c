...
const uint32_t xqc_bbr2_probertt_win_size_us = 20000000;
...
static const uint64_t LOG_INTERVAL = 5000;
static uint64_t log_cnt = 0;

/* 文件标识符*/
static FILE *bbr_log_fp = NULL;
static FILE *bbr_log_fp_mode = NULL;
...
void xqc_bbr_log_bbr_init(xqc_bbr_t *bbr, xqc_sample_t *sampler);
                              ...
                              xqc_sample_t *sampler, uint64_t bw)
{
    ...
    if (bbr->lt_bw)
    {
        ...
        if (diff <= (uint64_t)(bbr->lt_bw * xqc_bbr_lt_bw_ratio) || diff <= (xqc_bbr_lt_bw_diff))
        {
...
xqc_bbr_update_lt_bw_sampling(xqc_bbr_t *bbr, xqc_sample_t *sampler)
{
    if (bbr->lt_use_bw)
    {
        if (bbr->mode == BBR_PROBE_BW && bbr->round_start && ++bbr->lt_rtt_cnt >= xqc_bbr_lt_bw_max_rtts)
        {
    ...
    if (acked_bytes == 0 || lost_bytes < (uint64_t)(acked_bytes * xqc_bbr_lt_bw_loss_thresh))
    {
...
xqc_bbr_init(void *cong_ctl, xqc_sample_t *sampler, xqc_cc_params_t cc_params)
{
    ...
    if (cc_params.customize_on)
    {
        ...
        bbr->initial_congestion_window =
            cc_params.init_cwnd >= XQC_BBR_MIN_WINDOW && cc_params.init_cwnd <= XQC_BBR_MAX_WINDOW
                ? cc_params.init_cwnd
                : XQC_BBR_INITIAL_WINDOW;
        bbr->min_cwnd = cc_params.min_cwnd >= XQC_BBR_MIN_WINDOW && cc_params.min_cwnd <= XQC_BBR_MAX_WINDOW
                            ? cc_params.min_cwnd
                            : xqc_bbr_min_cwnd;
    ...
    xqc_bbr_log_bbr_init(bbr, sampler);
...
xqc_bbr_update_bandwidth(xqc_bbr_t *bbr, xqc_sample_t *sampler)
{
    ...
    if (bbr->lt_bw_enabled && bbr->mode != BBR_STARTUP && bbr->mode != BBR_DRAIN)
    {
...
void xqc_bbr_log_bbr_init(xqc_bbr_t *bbr, xqc_sample_t *sampler)
{
    if (bbr_log_fp_mode == NULL)
    {
        bbr_log_fp_mode = fopen("/home/mxjpxk/xquic/xquic_info/bbr_init.log", "a"); // 打开日志文件（只打开一次）
        if (!bbr_log_fp_mode)
        {
            perror("Failed to open bbr_init.log");
            return;
        }
    }
    fprintf(bbr_log_fp_mode, "[BBR_INIT] time=%" PRIu64 " bbr=%p\n", xqc_monotonic_timestamp(), bbr);
}

/*打印输出*/
void xqc_bbr_log_metrics(xqc_bbr_t *bbr, xqc_sample_t *sampler)
{
    uint64_t ts = sampler->now;
    if (ts - last_log_time >= LOG_INTERVAL || log_cnt == 0)
    {
        if (bbr_log_fp == NULL)
        {
            bbr_log_fp = fopen("/home/mxjpxk/xquic/xquic_info/bbr_metrics.log", "a"); // 打开日志文件（只打开一次）
            if (!bbr_log_fp)
            {
                perror("Failed to open bbr_metrics.log");
                return;
            }
            fprintf(bbr_log_fp, "timestamp(us),prevent_timestamp(us),BBR,conn_ctl,mode,min_rtt(us),cwnd,bandwidth(bytes/s),pacing_rate\n");
        }
        fprintf(bbr_log_fp, "%" PRIu64 ",%" PRIu64 ", %p, %p, %u, %" PRIu64 ", %u, %" PRIu32 ", %f \n",
                ts,
                last_log_time,
                bbr,
                sampler->send_ctl->ctl_conn,
                bbr->mode,
                bbr->min_rtt,
...
xqc_bbr_is_next_cycle_phase(xqc_bbr_t *bbr, xqc_sample_t *sampler)
{
    ...
    {
        should_advance_gain_cycling = is_full_length && (sampler->loss || inflight >= xqc_bbr_target_cwnd(bbr,
                                                                                                          bbr->pacing_gain,
                                                                                                          xqc_bbr_max_bw(bbr)));
    }
    ...
    if (bbr->pacing_gain < 1.0)
    {
        should_advance_gain_cycling = is_full_length || (inflight <= xqc_bbr_target_cwnd(bbr, 1.0, xqc_bbr_max_bw(bbr)));
    }
...
xqc_update_ack_aggregation(xqc_bbr_t *bbr, xqc_sample_t *sampler)
{
    ...
    if (!xqc_bbr_extra_ack_gain || sampler->delivered <= 0 || sampler->interval <= 0 || sampler->acked <= 0)
    {
        return;
    }
    ...
    if (bbr->epoch_ack <= expected_ack || (bbr->epoch_ack + sampler->acked >= xqc_bbr_ack_epoch_acked_reset_thresh))
    {
        bbr->epoch_ack = 0;
...
xqc_bbr_check_full_bw_reached(xqc_bbr_t *bbr, xqc_sample_t *sampler)
{
     ...
     */
    if (!bbr->round_start || bbr->full_bandwidth_reached || (!bbr->ignore_app_limit && sampler->is_app_limited))
    {
        /*
         * In a live video scenario, the applimit state often occurs,
...
xqc_bbr_check_drain(xqc_bbr_t *bbr, xqc_sample_t *sampler)
{
    ...
    if (bbr->mode == BBR_DRAIN && sampler->bytes_inflight <= xqc_bbr_target_cwnd(bbr,
                                                                                 1.0,
                                                                                 xqc_bbr_max_bw(bbr)))
    {
#if XQC_BBR_RTTVAR_COMPENSATION_ENABLED
...
static void
xqc_bbr_enter_probe_rtt(xqc_bbr_t *bbr, xqc_sample_t *sampler)
{
...
xqc_bbr_update_min_rtt(xqc_bbr_t *bbr, xqc_sample_t *sampler)
{
...
#ifndef XQC_BBR_DISABLE_CWND_AI
        if (bbr->probe_rtt_min_us_stamp != bbr->min_rtt_stamp || min_rtt_expired)
        {
            /*
    ...
    if (probe_rtt_expired && bbr->mode != BBR_PROBE_RTT && !bbr->idle_restart)
    {
        xqc_bbr_enter_probe_rtt(bbr, sampler);
    ...
    if (bbr->mode == BBR_PROBE_RTT)
    {
        ...
        send_ctl->ctl_app_limited = (send_ctl->ctl_delivered + send_ctl->ctl_bytes_in_flight) ? (send_ctl->ctl_delivered + send_ctl->ctl_bytes_in_flight) : 1;
        ...
        if (!bbr->probe_rtt_round_done_stamp && (sampler->bytes_inflight <= xqc_bbr_probe_rtt_cwnd(bbr)))
        {
...
xqc_bbr_set_cwnd(xqc_bbr_t *bbr, xqc_sample_t *sampler)
{
    if (sampler->acked != 0)
    {
        ...
        if (!bbr->packet_conservation)
        {
            ...
            {
#ifndef XQC_BBR_DISABLE_CWND_AI
                if ((bbr->congestion_window + sampler->acked >= (target_cwnd + bbr->beyond_target_cwnd)) && sampler->send_ctl->ctl_is_cwnd_limited)
                {
...
xqc_bbr_on_ack(void *cong_ctl, xqc_sample_t *sampler)
{
    ...
    xqc_bbr_log_metrics(bbr, sampler);
